...
private:
    ...
    Job* job;
    Broker* broker;
    HANDLE sandboxProcess;

public:

    // Instantiate and assign the Job on construction
    Sandbox() : job(new Job(*this)), broker(nullptr), sandboxProcess(nullptr) {};

    // Free the Job and Broker on destruction
    ~Sandbox();
    ...
    // Wait for the sandbox runner process to exit and return its exit code.
    DWORD WaitForSandbox(DWORD &dwExitCode);
